...
const PRIVAT_API_URL = "https://api.privatbank.ua/p24api/pubinfo?json&exchange&coursid=5"
...
async function getMono(){
    ...
    currenciesInfo.forEach((currencyInfo) => {
        if (currencyInfo.currencyCodeA === 840 && currencyInfo.currencyCodeB === 980){
            currencyRates.monoUsdSell = Number(currencyInfo.rateSell)
            currencyRates.monoUsdBuy = Number(currencyInfo.rateBuy)
        }
        if (currencyInfo.currencyCodeA === 978 && currencyInfo.currencyCodeB === 980){
...
export async function getUsdMessage(){
    const {monoUsdSell, monoUsdBuy, privatUsdSell, privatUsdBuy} = await getExchange()
...
export async function getEurMessage(){
    const {monoEurSell, monoEurBuy, privatEurSell, privatEurBuy} = await getExchange()
